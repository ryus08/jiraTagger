...
if [[ -z "$4" ]]; then
    func="JiraTaggerDev${func}Function"
fi
...
if [[ -z "$3" ]]; then
    ...
    sam.cmd local invoke -d 8997 --debugger-path ./scripts/windows --debug  --region us-east-1 --event ${event} ${func}
else
    echo "Docker Network"
    network="$3"
    echo "sam local invoke -d 8997 --debugger-path ./scripts/linux --debug  --region us-east-1 --docker-network ${network} --event ${event} ${func}"
    sam.cmd local invoke -d 8997 --debugger-path ./scripts/windows --debug  --region us-east-1 --docker-network ${network} --event ${event} ${func}
fi